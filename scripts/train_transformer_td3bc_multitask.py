#!/usr/bin/env python3
"""
Train a transformer TD3+BC policy on multiple expert datasets.

This script trains a single multitask policy using transformer networks with
semantic tokens and AdaLN conditioning on the first 14 training tasks from expert data,
then evaluates on all 32 test tasks.
"""
...
# Import transformer networks
sys.path.insert(0, str(Path(__file__).parent))
from transformer_networks import (
    TransformerEncoder,
    TransformerEncoderWithAction,
    create_transformer_encoderfactory_params
)
...
def load_multitask_dataset(
    ...
    IMPORTANT: Task IDs are kept in observations for transformer networks (used as conditioning).
    """
...
class TransformerActor(nn.Module):
    """Actor with transformer encoder"""
    ...
    def __init__(self, encoder_kwargs, observation_shape, max_action):
        ...
        self.encoder = TransformerEncoder(
            ...
            hidden_size=encoder_kwargs.get("hidden_size", 256),
            depth=encoder_kwargs.get("depth", 6),
            num_heads=encoder_kwargs.get("num_heads", 8),
            mlp_ratio=encoder_kwargs.get("mlp_ratio", 4.0),
            dropout=encoder_kwargs.get("dropout", 0.0),
        )
...
class TransformerCritic(nn.Module):
    """Critic with transformer encoder (with action)"""
    
    def __init__(self, encoder_kwargs, observation_shape, action_size):
        super().__init__()
        self.encoder = TransformerEncoderWithAction(
            observation_shape=observation_shape,
...
class TransformerTD3BC:
    """TD3+BC with transformer networks"""
    ...
    def __init__(
        ...
        actor_encoder_kwargs = create_transformer_encoderfactory_params(
            with_action=False, 
        ...
        # Create transformer critics
        critic_encoder_kwargs = create_transformer_encoderfactory_params(
            with_action=True, 
            output_dim=1,
            hidden_size=hidden_size,
            depth=depth,
        ...
        self.critic_1_target = TransformerCritic(
            critic_encoder_kwargs, (state_dim,), action_dim
        ).to(device)
        ...
        self.critic_2_target = TransformerCritic(
            critic_encoder_kwargs, (state_dim,), action_dim
        ).to(device)
...
def evaluate_policy(agent, task_list, n_episodes=10, use_task_id_obs=True):
    ...
    IMPORTANT: use_task_id_obs=True because transformer networks need task IDs for conditioning.
    """
    results = {}
    ...
    for task in tqdm(task_list, desc="Evaluating tasks"):
        ...
        # Create environment WITH task IDs for transformer networks (used as conditioning)
...
def main():
    ...
    parser.add_argument('--batch_size', type=int, default=1792,
                       help='Batch size (reduced for transformer memory efficiency)')
    ...
    print(f"\n{'='*80}")
    print("TRANSFORMER TD3+BC MULTITASK TRAINING")
    print(f"{'='*80}")
    ...
    # Create agent with transformer networks
    state_dim = 93  # CompoSuite observation dimension WITH task IDs
    action_dim = 8
    max_action = 1.0
    
    agent = TransformerTD3BC(
        state_dim=state_dim,
        action_dim=action_dim,
        max_action=max_action,
        device=args.device,
        hidden_size=72,  # Transformer hidden size (reduced for efficiency, divisible by 4)
        depth=1,  # Number of transformer blocks (reduced for efficiency)
        num_heads=4,  # Attention heads
        mlp_ratio=1.20,  # MLP expansion ratio
        dropout=0.0,  # Dropout rate
    )
    print(f"✓ Created transformer TD3+BC agent")
    ...
    for t in tqdm(range(1, args.max_timesteps + 1), desc="Training"):
        ...
        if t % args.eval_freq == 0:
            ...
            eval_results = evaluate_policy(
                ...
                use_task_id_obs=True  # Transformer networks need task IDs for conditioning
            )
