#!/usr/bin/env python3
"""
Run Transformer TD3+BC Baseline Pipeline

This script submits SLURM jobs for training transformer TD3+BC policies,
monitors their completion, and analyzes results.

Uses transformer networks with semantic tokens and AdaLN conditioning.
"""
...
class BaselineConfig:
    ...
    results_path: str = '/home/quanpham/iterative-comp-rl-generation/results/transformer_baseline'
    ...
    batch_size: int = 1792  # Reduced for transformer memory efficiency
...
class TransformerBaselinePipeline:
    """Pipeline for running transformer TD3+BC baseline"""
    ...
    def setup_directories(self):
        ...
        dirs_to_create = [
            ...
            self.base_path / "scripts" / "transformer_baseline_logs",
            self.results_path,
        ]
    ...
    def submit_training_jobs(self) -> List[Dict]:
        ...
        for seed in self.config.seeds:
            ...
            script_path = self.base_path / f"job_transformer_baseline_seed{seed}.sh"
    ...
    def _build_training_script(self, seed: int, retry_context: dict = None) -> str:
        ...
        job_name = f"transformer_baseline_seed{seed}"
        
        return f"""#!/bin/bash
#SBATCH --job-name={job_name}
#SBATCH --output=scripts/transformer_baseline_logs/%j_{job_name}.out
#SBATCH --mem={memory}G
#SBATCH --gpus=1
...
python {self.config.base_path}/scripts/train_transformer_td3bc_multitask.py \\
...
"""
    ...
    def _get_job_failure_reason(self, job_id: str) -> str:
        ...
        try:
            ...
            log_pattern = f"scripts/transformer_baseline_logs/{job_id}_*.out"
    ...
    def analyze_results(self) -> pd.DataFrame:
        ...
        for seed in self.config.seeds:
            ...
            log_dir = self.base_path / "scripts" / "transformer_baseline_logs"
            log_pattern = f"*_transformer_baseline_seed{seed}.out"
        ...
        # Save to CSV
        csv_path = self.results_path / "transformer_baseline_results.csv"
        df.to_csv(csv_path, index=False)
        print(f"\n✓ Saved results table to: {csv_path}")
        
        # Print table
        print(f"\n{'='*80}")
        print("TRANSFORMER TD3+BC BASELINE RESULTS")
        print(f"{'='*80}")
    ...
    def run(self):
        ...
        print(f"\n{'='*80}")
        print("TRANSFORMER TD3+BC BASELINE PIPELINE")
        print(f"{'='*80}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Transformer TD3+BC Baseline Pipeline"
    )
    ...
    # Run pipeline
    pipeline = TransformerBaselinePipeline(config)
    pipeline.run()
