...
if __name__ == '__main__':
    ...
    # Initialize denoiser network based on denoiser type.
    # Use construct_diffusion_model to handle all model types and configurations from gin
    diffusion_temp = construct_diffusion_model(inputs=inputs, denoiser=args.denoiser, cond_dim=indicators.shape[1])
    model = diffusion_temp.net  # Extract the denoiser from the diffusion wrapper
    model = accelerator.prepare(model)

    # Load checkpoint.
    checkpoint_path = os.path.join(results_folder, 'model-100000.pt') #for fast testing, we reduced from 100000 to 10
    ...
    if torch.__version__ >= "2.0.0":
        print("Using torch.compile() for faster generation")
        diffusion = torch.compile(diffusion)
    else:
        print(f"PyTorch {torch.__version__} detected, skipping compile")
