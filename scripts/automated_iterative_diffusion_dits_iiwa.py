...
class IterationConfig:
    ...
    denoiser: str = 'dit'
    num_train: int = 14  # Updated for IIWA tasks (14 total training tasks)
...
class IterativeDiscoveryPipeline:
    ...
    def get_train_and_middle_tasks_from_file(self) -> Tuple[List[str], List[str]]:
        ...
        with open(task_file, 'r') as f:
            ...
            actual_train_tasks = full_train_tasks[:self.config.num_train]  # First 32 tasks
            middle_tasks = full_train_tasks[self.config.num_train:]        # Tasks remaining
...
python {self.config.base_path}/scripts/train_augmented_diffusion.py \\
    ...
    --gin_config_files {self.config.base_path}/config/diffusion_dit.gin \\
    ...
    --dataset_type expert \\
    --experiment_type component-wise \\
...
python {self.config.base_path}/scripts/generate_augmented_data_dits.py \\
    --base_data_path {self.config.data_path} \\
    --base_results_folder {self.config.results_path} \\
    --gin_config_files {self.config.base_path}/config/diffusion_dit.gin \\
    --denoiser {self.config.denoiser} \\
...
"""
    ...
    def _create_best_testtask_dataset(self):
        ...
        best_dataset_name = f"best_testtask_dataset/dit_tasklist{self.config.task_list_seed}_train{self.config.num_train}_diffusionseed{self.config.diffusion_seed}_iters{max_iteration}"
    ...
    def _parse_policy_logs(self, iteration: int, task_type: str, successful_job_ids: Optional[Set[str]] = None) -> Dict[str, float]:
        ...
        for log_file in log_dir.glob("*.out"):
            ...
            match = re.search(r'(\d+)_dit_tasklist(\d+)_train(\d+)_diffusionseed(\d+)_td3_bc_(\w+)_(\w+)_(\w+)_(\w+)_RLseed(\d+)_(\d+)\.out', log_file.name)
            if not match:
                continue
    ...
    def _create_comparison_csv(self, iteration: int, task_type: str, seeds: tuple):
        ...
        for log_file in log_dir.glob("*.out"):
            ...
            import re
            match = re.match(
                r'(\d+)_dit_tasklist(\d+)_train(\d+)_diffusionseed(\d+)_td3_bc_(\w+)_(\w+)_(\w+)_(\w+)_RLseed(\d+)_(\d+)\.out',
                log_file.name
            )
            if not match:
                continue
...
def main():
    ...
    parser.add_argument('--denoiser', type=str, default='dit',
                       help='Denoiser type')
